...
# Check Data
function CheckData() {
    exclude_list_checked=($(cat ./exclude_list_unchecked.tmp | sed "s/[[:space:]]//g;s/\#.*//g" | grep "http\|https\|udp\|wss" | grep -v "announce+\|announcehttp\|announcehttps\|announceudp\|announcewss" | awk '{ match( $0, /.*\:+[0-9]+\/+announce+/ ); print substr( $0, RSTART, RLENGTH ) }' | sort | uniq | awk "{ print $2 }"))
    full_list_checked=($(cat ./full_list_unchecked.tmp | sed "s/[[:space:]]//g;s/\#.*//g" | grep "http\|https\|udp\|wss" | grep -v "announce+\|announcehttp\|announcehttps\|announceudp\|announcewss" | awk '{ match( $0, /.*\:+[0-9]+\/+announce+/ ); print substr( $0, RSTART, RLENGTH ) }' | sort | uniq | awk "{ print $2 }"))
